...
class InMemoryDataSetIteratorTest {

    @Test
    fun simpleLabel() {
        val iterator = InMemoryDataSetIterator(csvToXY(resource("cpu.onehot.data.csv"), "class"), 10)
        assertEquals(
    ...
    @Test
    fun multiLabel() {
        val iterator = InMemoryDataSetIterator(csvToXY(resource("iris.onehot.csv"), "class"), 10)
        assertEquals(
